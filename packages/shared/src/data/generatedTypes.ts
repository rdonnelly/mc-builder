// generated types from marvelsdb-json-data, do not edit

export enum FactionCodes {
  AGGRESSION = 'aggression',
  BASIC = 'basic',
  CAMPAIGN = 'campaign',
  ENCOUNTER = 'encounter',
  HERO = 'hero',
  JUSTICE = 'justice',
  LEADERSHIP = 'leadership',
  POOL = 'pool',
  PROTECTION = 'protection',
}

export enum PackCodes {
  ANGEL = 'angel',
  ANT = 'ant',
  AOA = 'aoa',
  AOS = 'aos',
  BKW = 'bkw',
  BP = 'bp',
  CAP = 'cap',
  CORE = 'core',
  CYCLOPS = 'cyclops',
  DEADPOOL = 'deadpool',
  DRAX = 'drax',
  DRS = 'drs',
  FALCON = 'falcon',
  GAM = 'gam',
  GAMBIT = 'gambit',
  GMW = 'gmw',
  GOB = 'gob',
  HLK = 'hlk',
  HOOD = 'hood',
  ICEMAN = 'iceman',
  IRONHEART = 'ironheart',
  JUBILEE = 'jubilee',
  MAGNETO = 'magneto',
  MOJO = 'mojo',
  MSM = 'msm',
  MTS = 'mts',
  MUT_GEN = 'mut_gen',
  NCRAWLER = 'ncrawler',
  NEBU = 'nebu',
  NEXT_EVOL = 'next_evol',
  NOVA = 'nova',
  PHOENIX = 'phoenix',
  PSYLOCKE = 'psylocke',
  QSV = 'qsv',
  ROGUE = 'rogue',
  RON = 'ron',
  SCW = 'scw',
  SILK = 'silk',
  SM = 'sm',
  SPDR = 'spdr',
  SPIDERHAM = 'spiderham',
  STLD = 'stld',
  STORM = 'storm',
  THOR = 'thor',
  TOAFK = 'toafk',
  TRORS = 'trors',
  TWC = 'twc',
  VALK = 'valk',
  VISION = 'vision',
  VNM = 'vnm',
  WARM = 'warm',
  WINTER = 'winter',
  WOLV = 'wolv',
  WSP = 'wsp',
  X23 = 'x23',
}

export enum SetCodes {
  A_I_M__ABDUCTION = 'a.i.m._abduction',
  A_I_M__SCIENCE = 'a.i.m._science',
  ABSORBING_MAN = 'absorbing_man',
  ACOLYTES = 'acolytes',
  AGE_OF_APOCALYPSE = 'age_of_apocalypse',
  ANACHRONAUTS = 'anachronauts',
  ANGEL = 'angel',
  ANGEL_NEMESIS = 'angel_nemesis',
  ANT = 'ant',
  ANT_NEMESIS = 'ant_nemesis',
  AOA_BASIC_CAMPAIGN = 'aoa_basic_campaign',
  AOA_CAMPAIGN = 'aoa_campaign',
  AOA_MISSION = 'aoa_mission',
  APOCALYPSE = 'apocalypse',
  ARCADE = 'arcade',
  ARMADILLO = 'armadillo',
  ARMIES_OF_TITAN = 'armies_of_titan',
  A_MESS_OF_THINGS = 'a_mess_of_things',
  BADOON_HEADHUNTER = 'badoon_headhunter',
  BAD_PUBLICITY = 'bad_publicity',
  BAND_OF_BADOON = 'band_of_badoon',
  BARON_ZEMO = 'baron_zemo',
  BATROC = 'batroc',
  BATROCS_BRIGADE = 'batrocs_brigade',
  BEASTY_BOYS = 'beasty_boys',
  BISHOP = 'bishop',
  BISHOP_NEMESIS = 'bishop_nemesis',
  BLACK_ORDER = 'black_order',
  BLACK_PANTHER = 'black_panther',
  BLACK_PANTHER_NEMESIS = 'black_panther_nemesis',
  BLACK_PANTHER_SHURI = 'black_panther_shuri',
  BLACK_PANTHER_SHURI_NEMESIS = 'black_panther_shuri_nemesis',
  BLACK_TOM_CASSIDY = 'black_tom_cassidy',
  BLACK_WIDOW = 'black_widow',
  BLACK_WIDOW_NEMESIS = 'black_widow_nemesis',
  BLACK_WIDOW_VILLAIN = 'black_widow_villain',
  BLUE_MOON = 'blue_moon',
  BOMB_SCARE = 'bomb_scare',
  BRAWLER = 'brawler',
  BROTHERHOOD = 'brotherhood',
  BROTHERHOOD_OF_BADOON = 'brotherhood_of_badoon',
  BROTHERS_GRIMM = 'brothers_grimm',
  BULLDOZER = 'bulldozer',
  CABLE = 'cable',
  CABLE_NEMESIS = 'cable_nemesis',
  CAPTAIN_AMERICA = 'captain_america',
  CAPTAIN_AMERICA_NEMESIS = 'captain_america_nemesis',
  CAPTAIN_MARVEL = 'captain_marvel',
  CAPTAIN_MARVEL_NEMESIS = 'captain_marvel_nemesis',
  CELESTIAL_TECH = 'celestial_tech',
  CHALLENGE = 'challenge',
  CHILDREN_OF_THANOS = 'children_of_thanos',
  CITY_IN_CHAOS = 'city_in_chaos',
  CLAN_AKKABA = 'clan_akkaba',
  COLOSSUS = 'colossus',
  COLOSSUS_NEMESIS = 'colossus_nemesis',
  COMMANDER = 'commander',
  COMMUNITY_SERVICE = 'community_service',
  CRAZY_GANG = 'crazy_gang',
  CRIME = 'crime',
  CROSSBONES = 'crossbones',
  CROSSFIRE_CREW = 'crossfire_crew',
  CYCLOPS = 'cyclops',
  CYCLOPS_NEMESIS = 'cyclops_nemesis',
  DARK_BEAST = 'dark_beast',
  DARK_RIDERS = 'dark_riders',
  DEADPOOL = 'deadpool',
  DEADPOOL_NEMESIS = 'deadpool_nemesis',
  DEATHSTRIKE = 'deathstrike',
  DEFENDER = 'defender',
  DOCTOR_STRANGE = 'doctor_strange',
  DOCTOR_STRANGE_NEMESIS = 'doctor_strange_nemesis',
  DOMINO = 'domino',
  DOMINO_NEMESIS = 'domino_nemesis',
  DOWN_TO_EARTH = 'down_to_earth',
  DRAX = 'drax',
  DRAX_NEMESIS = 'drax_nemesis',
  DREADPOOL = 'dreadpool',
  DYSTOPIAN_NIGHTMARE = 'dystopian_nightmare',
  EBONY_MAW = 'ebony_maw',
  ENCHANTRESS = 'enchantress',
  EN_SABAH_NUR = 'en_sabah_nur',
  ESCAPE_THE_MUSEUM = 'escape_the_museum',
  EXECUTIVE_BOARD_EVIDENCE = 'executive_board_evidence',
  EXODUS = 'exodus',
  EXPCAMP = 'expcamp',
  EXPERT = 'expert',
  EXPERT_II = 'expert_ii',
  EXPER_WEAPON = 'exper_weapon',
  EXP_KANG = 'exp_kang',
  EXTREME_MEASURES = 'extreme_measures',
  EXTREME_RISK = 'extreme_risk',
  FALCON = 'falcon',
  FALCON_NEMESIS = 'falcon_nemesis',
  FANTASY = 'fantasy',
  FLIGHT = 'flight',
  FOUR_HORSEMEN = 'four_horsemen',
  FROSTBITE = 'frostbite',
  FROST_GIANTS = 'frost_giants',
  FUTURE_PAST = 'future_past',
  GALACTIC_ARTIFACTS = 'galactic_artifacts',
  GAM = 'gam',
  GAMBIT = 'gambit',
  GAMBIT_NEMESIS = 'gambit_nemesis',
  GAM_NEMESIS = 'gam_nemesis',
  GENOSHA = 'genosha',
  GHOST_SPIDER = 'ghost_spider',
  GHOST_SPIDER_NEMESIS = 'ghost_spider_nemesis',
  GOBLIN_GEAR = 'goblin_gear',
  GOBLIN_GIMMICKS = 'goblin_gimmicks',
  GRAVITATIONAL_PULL = 'gravitational_pull',
  GROOT = 'groot',
  GROOT_NEMESIS = 'groot_nemesis',
  GROWING_STRONG = 'growing_strong',
  GUERRILLA_TACTICS = 'guerrilla_tactics',
  HARD_SOUND = 'hard_sound',
  HAWKEYE = 'hawkeye',
  HAWKEYE_NEMESIS = 'hawkeye_nemesis',
  HELA = 'hela',
  HELLFIRE = 'hellfire',
  HOPE_SUMMERS = 'hope_summers',
  HORROR = 'horror',
  HOUNDS = 'hounds',
  HULK = 'hulk',
  HULK_NEMESIS = 'hulk_nemesis',
  HYDRA_ASSAULT = 'hydra_assault',
  HYDRA_CAMP = 'hydra_camp',
  HYDRA_PATROL = 'hydra_patrol',
  ICEMAN = 'iceman',
  ICEMAN_NEMESIS = 'iceman_nemesis',
  INFILTRATE_THE_MUSEUM = 'infiltrate_the_museum',
  INFINITES = 'infinites',
  INFINITY_GAUNTLET = 'infinity_gauntlet',
  INHERITORS = 'inheritors',
  INVOCATION = 'invocation',
  IRONHEART = 'ironheart',
  IRONHEART_NEMESIS = 'ironheart_nemesis',
  IRONSPIDER_SINISTER = 'ironspider_sinister',
  IRON_MAN = 'iron_man',
  IRON_MAN_NEMESIS = 'iron_man_nemesis',
  JUBILEE = 'jubilee',
  JUBILEE_NEMESIS = 'jubilee_nemesis',
  JUGGERNAUT = 'juggernaut',
  KANG = 'kang',
  KLAW = 'klaw',
  KREE_FANATIC = 'kree_fanatic',
  KREE_MILITANT = 'kree_militant',
  LEGIONS_OF_HEL = 'legions_of_hel',
  LEGIONS_OF_HYDRA = 'legions_of_hydra',
  LOKI = 'loki',
  LONGSHOT = 'longshot',
  M_O_D_O_K_ = 'm.o.d.o.k.',
  MAGIK = 'magik',
  MAGIK_NEMESIS = 'magik_nemesis',
  MAGNETO = 'magneto',
  MAGNETO_NEMESIS = 'magneto_nemesis',
  MAGNETO_VILLAIN = 'magneto_villain',
  MAGOG = 'magog',
  MANSION_ATTACK = 'mansion_attack',
  MARAUDERS = 'marauders',
  MARIA_HILL = 'maria_hill',
  MARIA_HILL_NEMESIS = 'maria_hill_nemesis',
  MASTERS_OF_EVIL = 'masters_of_evil',
  MASTER_MOLD = 'master_mold',
  MENAGERIE_MEDLEY = 'menagerie_medley',
  MILITARY_GRADE = 'military_grade',
  MISTER_HYDE = 'mister_hyde',
  MISTER_SINISTER = 'mister_sinister',
  MOJO = 'mojo',
  MORLOCK_SIEGE = 'morlock_siege',
  MOT = 'mot',
  MS_MARVEL = 'ms_marvel',
  MS_MARVEL_NEMESIS = 'ms_marvel_nemesis',
  MTS_CAMPAIGN = 'mts_campaign',
  MUTAGEN_FORMULA = 'mutagen_formula',
  MUTANT_INSURRECTION = 'mutant_insurrection',
  MUTANT_SLAYERS = 'mutant_slayers',
  MUT_GEN_CAMPAIGN = 'mut_gen_campaign',
  MYSTERIO = 'mysterio',
  MYSTIQUE = 'mystique',
  NASTY_BOYS = 'nasty_boys',
  NEBU = 'nebu',
  NEBULA = 'nebula',
  NEBU_NEMESIS = 'nebu_nemesis',
  NEXT_EVOL_CAMPAIGN = 'next_evol_campaign',
  NICK_FURY = 'nick_fury',
  NICK_FURY_NEMESIS = 'nick_fury_nemesis',
  NIGHTCRAWLER = 'nightcrawler',
  NIGHTCRAWLER_NEMESIS = 'nightcrawler_nemesis',
  NOVA = 'nova',
  NOVA_NEMESIS = 'nova_nemesis',
  ON_THE_RUN = 'on_the_run',
  OSBORN_TECH = 'osborn_tech',
  OVERSEER = 'overseer',
  PALE_LITTLE_SPIDER = 'pale_little_spider',
  PEACEKEEPER = 'peacekeeper',
  PERSONAL_NIGHTMARE = 'personal_nightmare',
  PHOENIX = 'phoenix',
  PHOENIX_NEMESIS = 'phoenix_nemesis',
  PILEDRIVER = 'piledriver',
  POWER_DRAIN = 'power_drain',
  POWER_OF_THE_ATOM = 'power_of_the_atom',
  POWER_STONE = 'power_stone',
  PRELATES = 'prelates',
  PROJECT_WIDEAWAKE = 'project_wideawake',
  PSYLOCKE = 'psylocke',
  PSYLOCKE_NEMESIS = 'psylocke_nemesis',
  QSV = 'qsv',
  QSV_NEMESIS = 'qsv_nemesis',
  RANSACKED_ARMORY = 'ransacked_armory',
  REAVERS = 'reavers',
  RED_SKULL = 'red_skull',
  RHINO = 'rhino',
  RISKY_BUSINESS = 'risky_business',
  ROCKET = 'rocket',
  ROCKET_NEMESIS = 'rocket_nemesis',
  ROGUE = 'rogue',
  ROGUE_NEMESIS = 'rogue_nemesis',
  RONAN = 'ronan',
  RUNNING_INTERFERENCE = 'running_interference',
  S_H_I_E_L_D_ = 's.h.i.e.l.d.',
  S_H_I_E_L_D__EXECUTIVE_BOARD = 's.h.i.e.l.d._executive_board',
  SABRETOOTH = 'sabretooth',
  SANDMAN = 'sandman',
  SAURON = 'sauron',
  SAVAGE_LAND = 'savage_land',
  SCI_FI = 'sci-fi',
  SCIENTIST_SUPREME = 'scientist_supreme',
  SCW = 'scw',
  SCW_NEMESIS = 'scw_nemesis',
  SENTINELS = 'sentinels',
  SHADOWCAT = 'shadowcat',
  SHADOWCAT_NEMESIS = 'shadowcat_nemesis',
  SHADOW_KING = 'shadow_king',
  SHE_HULK = 'she_hulk',
  SHE_HULK_NEMESIS = 'she_hulk_nemesis',
  SHIELD_TECH = 'shield_tech',
  SHIP_COMMAND = 'ship_command',
  SILK = 'silk',
  SILK_NEMESIS = 'silk_nemesis',
  SINISTER_ASSAULT = 'sinister_assault',
  SINISTER_SIX = 'sinister_six',
  SINISTER_SYNDICATE = 'sinister_syndicate',
  SITCOM = 'sitcom',
  SNITCHES_GET_STITCHES = 'snitches_get_stitches',
  SPACE_PIRATES = 'space_pirates',
  SPDR = 'spdr',
  SPDR_NEMESIS = 'spdr_nemesis',
  SPECTRUM = 'spectrum',
  SPECTRUM_NEMESIS = 'spectrum_nemesis',
  SPIDERHAM = 'spiderham',
  SPIDERHAM_NEMESIS = 'spiderham_nemesis',
  SPIDER_MAN = 'spider_man',
  SPIDER_MAN_MORALES = 'spider_man_morales',
  SPIDER_MAN_MORALES_NEMESIS = 'spider_man_morales_nemesis',
  SPIDER_MAN_NEMESIS = 'spider_man_nemesis',
  SPIDER_WOMAN = 'spider_woman',
  SPIDER_WOMAN_NEMESIS = 'spider_woman_nemesis',
  SPIRAL = 'spiral',
  STANDARD = 'standard',
  STANDARD_II = 'standard_ii',
  STANDARD_III = 'standard_iii',
  STATE_OF_EMERGENCY = 'state_of_emergency',
  STLD = 'stld',
  STLD_NEMESIS = 'stld_nemesis',
  STORM = 'storm',
  STORM_NEMESIS = 'storm_nemesis',
  STREETS_OF_MAYHEM = 'streets_of_mayhem',
  STRYFE = 'stryfe',
  SUPERSONIC = 'supersonic',
  SUPER_STRENGTH = 'super_strength',
  SYMBIOTIC_STRENGTH = 'symbiotic_strength',
  TASKMASTER = 'taskmaster',
  TECHNO = 'techno',
  TELEPATHY = 'telepathy',
  TEMPORAL = 'temporal',
  THANOS = 'thanos',
  THE_DOOMSDAY_CHAIR = 'the_doomsday_chair',
  THE_HOOD = 'the_hood',
  THE_LEAPER = 'the_leaper',
  THE_MARKET = 'the_market',
  THOR = 'thor',
  THOR_NEMESIS = 'thor_nemesis',
  THUNDERBALL = 'thunderball',
  THUNDERBOLTS = 'thunderbolts',
  TOWER_DEFENSE = 'tower_defense',
  ULTRON = 'ultron',
  UNDER_ATTACK = 'under_attack',
  UNUS = 'unus',
  VALK = 'valk',
  VALK_NEMESIS = 'valk_nemesis',
  VENOM = 'venom',
  VENOM_GOBLIN = 'venom_goblin',
  VISION = 'vision',
  VISION_NEMESIS = 'vision_nemesis',
  VNM = 'vnm',
  VNM_NEMESIS = 'vnm_nemesis',
  WARLOCK = 'warlock',
  WARLOCK_NEMESIS = 'warlock_nemesis',
  WARM = 'warm',
  WARM_NEMESIS = 'warm_nemesis',
  WEAP_MASTER = 'weap_master',
  WEATHER = 'weather',
  WESTERN = 'western',
  WHISPERS_OF_PARANOIA = 'whispers_of_paranoia',
  WHITEOUT = 'whiteout',
  WINTER_SOLDIER = 'winter_soldier',
  WINTER_SOLDIER_NEMESIS = 'winter_soldier_nemesis',
  WOLVERINE = 'wolverine',
  WOLVERINE_NEMESIS = 'wolverine_nemesis',
  WRECKER = 'wrecker',
  WRECKING_CREW = 'wrecking_crew',
  WRECKING_CREW_MODULAR = 'wrecking_crew_modular',
  WSP = 'wsp',
  WSP_NEMESIS = 'wsp_nemesis',
  X23 = 'x23',
  X23_NEMESIS = 'x23_nemesis',
  ZERO_TOLERANCE = 'zero_tolerance',
  ZOLA = 'zola',
  ZZZAX = 'zzzax',
}

export enum SetTypeCodes {
  EVIDENCE = 'evidence',
  EXPERT = 'expert',
  HERO = 'hero',
  INVOCATION = 'invocation',
  MODULAR = 'modular',
  NEMESIS = 'nemesis',
  STANDARD = 'standard',
  VILLAIN = 'villain',
  WEATHER = 'weather',
}

export enum TypeCodes {
  ALLY = 'ally',
  ALTER_EGO = 'alter_ego',
  ATTACHMENT = 'attachment',
  ENVIRONMENT = 'environment',
  EVENT = 'event',
  EVIDENCE_MEANS = 'evidence_means',
  EVIDENCE_MOTIVE = 'evidence_motive',
  EVIDENCE_OPPORTUNITY = 'evidence_opportunity',
  HERO = 'hero',
  MAIN_SCHEME = 'main_scheme',
  MINION = 'minion',
  OBLIGATION = 'obligation',
  PLAYER_SIDE_SCHEME = 'player_side_scheme',
  RESOURCE = 'resource',
  SIDE_SCHEME = 'side_scheme',
  SUPPORT = 'support',
  TREACHERY = 'treachery',
  UPGRADE = 'upgrade',
  VILLAIN = 'villain',
}
